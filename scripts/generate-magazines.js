...
function buildNightIssue() {
  if (!fs.existsSync(nightIssueDir)) return [];

  return fs.readdirSync(nightIssueDir, { withFileTypes: true })
    .filter((entry) => entry.isFile())
    .map((entry) => entry.name)
    .filter((fileName) => imageExtensions.has(path.extname(fileName).toLowerCase()))
    .sort(naturalPageSort);
...
const nightIssue = buildNightIssue();
...
globalThis.EDEN_NIGHT_ISSUE = ${JSON.stringify(nightIssue, null, 2)};
...
console.log(`Generated ${path.relative(rootDir, outputFile)} with ${essentials.length} essentials, ${nightIssue.length} night issue images, and ${magazines.length} magazines.`);
